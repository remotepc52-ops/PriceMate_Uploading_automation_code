...
#     '_cfuvid': 'obvU1s0ppC7n5ACi8B20ELLuFsBlGhduqIAckWFy.QA-1761027790688-0.0.1.1-604800000',
# }
#
# headers = {
...
#     'correlationid': '6c4350a0-ae46-11f0-9923-053e9c76de31',
#     'country': 'AU',
#     'currency': 'AUD',
#     'iscomposition': 'Y',
#     'locale': 'en_AU',
...
#         url = 'https://www.bunnings.com.au/_apis/v1/products/regionalCategory?depth=3'
#
...
#             level2_list = level1.get("levels", [])
#             has_level3 = any("levels" in l2 and l2["levels"] for l2 in level2_list)
#
#             if has_level3:  # insert only level 3
#                 for level2 in level2_list:
#                     for level3 in level2.get("levels", []):
#                         path3 = level3.get("internalPath")
#                         if path3:
#                             cate_url = f"https://www.bunnings.com.au/products{path3}"
#                             hash_id = self.generate_hash_id(cate_url, self.retailer, self.region)
#                             self.category_input.update_one(
#                                 {"_id": hash_id},
#                                 {"$set": {"url": cate_url, "Status": "Pending", "retailer": self.retailer,
#                                           "region": self.region}},
#                                 upsert=True
#                             )
#                             print(f"inserted: {cate_url}")
#
#             elif level2_list:  # no level 3, insert level 2
#                 for level2 in level2_list:
#                     path2 = level2.get("internalPath")
#                     if path2:
#                         cate_url = f"https://www.bunnings.com.au/products{path2}"
#                         hash_id = self.generate_hash_id(cate_url, self.retailer, self.region)
#                         self.category_input.update_one(
#                             {"_id": hash_id},
#                             {"$set": {"url": cate_url, "Status": "Pending", "retailer": self.retailer,
...
#                         print(f"inserted: {cate_url}")
#
#             else:  # no level 2 or 3, insert level 1
#                 path1 = level1.get("internalPath")
#                 if path1:
#                     cate_url = f"https://www.bunnings.com.au/products{path1}"
#                     hash_id = self.generate_hash_id(cate_url, self.retailer, self.region)
#                     self.category_input.update_one(
#                         {"_id": hash_id},
#                         {"$set": {"url": cate_url, "Status": "Pending", "retailer": self.retailer,
...
#     execute( "scrapy crawl bunnings_cate -a retailer=bunnings_au -a region=au -a Type=eshop -a RetailerCode=bunnings_au".split())