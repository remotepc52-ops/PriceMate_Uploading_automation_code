...
class PricemateBaseSpider(Spider):
    ...
    def __init__(self, retailer, region, Type, RetailerCode, start=0, end=100000, lazada_pdp=False, *args, **kwargs):
        ...
        if self.lazada_pdp:
            self.base_path =  f"E:\\Data\\Crawl_Data_Collection\\Lazada_Master_PDP\\{self.region}"
        elif 'shopee' in self.retailer:
            self.base_path =  f"E:\\Data\\crawl_data_collection\\Master_PriceMate\\{self.retailer}\\{self.today}"
        else:
            self.base_path = f"E:\\Data\\crawl_data_collection\\PriceMate\\{self.retailer}\\{self.today}"
