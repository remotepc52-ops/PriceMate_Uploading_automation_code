...
def Export_Data(Website):
    ...
    data = list(product_data.find({"Status": "Done"}).limit(100000))
    df = pd.DataFrame(data)

    # Specify the columns you want to export
    columns_to_export = ["ProductURL", "ProductCode", "Name", "Price", "WasPrice", "RRP", "Offer_info",
                         "per_unit_price", "Pack_size", "Barcode", "Category_Hierarchy", "Brand",
                         "Promo_Type", "is_available"]  # Example columns
